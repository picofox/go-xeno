...
// WaitWrite .
func (pd *PollDescriptor) WaitWrite(ctx context.Context) (err error) {
...
func (pd *PollDescriptor) onwrite(p Poll) error {
	select {
...
func (pd *PollDescriptor) onhup(p Poll) error {
	select {
	case <-pd.closeTrigger:
	default:
...
func (pd *PollDescriptor) detach() {
	if err := pd.operator.Control(PollDetach); err != nil {
		logger.Printf("NETPOLL: PollDescriptor detach operator failed: %v", err)
	}
}
